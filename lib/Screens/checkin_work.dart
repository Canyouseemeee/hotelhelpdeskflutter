import 'dart:async';
import 'dart:convert';
import 'package:hotelhelpdesk/Models/Checkin.dart';
...
class Checkin extends StatefulWidget {
  ...
  @override
  _CheckinState createState() => _CheckinState(news);
}

class _CheckinState extends State<Checkin> {
  String news;
  _CheckinState(this.news);
  bool _loading;
  ...
  static Future<List<Checkins>> getCheckin(String news) async {
    Map data = {
      'issuesid': news,
    };
    const String url = '${ApiUrl}'+"/api/issues-getstatus";
    try {
      final response = await http.post(url, body: data);
      if (response.statusCode == 200) {
        if (response.body.isNotEmpty) {
          final List<Checkins> checkin = checkinFromJson(response.body);
          return checkin;
        }
      } else {
  ...
  poststatus(String news) async {
    ...
    if (sharedPreferences.getString("token") != null) {
      ...
      var response = await http
          .post('${ApiUrl}'+"/api/issues-poststatus", body: data);
      if (response.statusCode == 200) {
  ...
  closedstatus(String news, String detail,String cid) async {
    ...
    if (sharedPreferences.getString("token") != null) {
      ...
      var response = await http.post(
          '${ApiUrl}'+"/api/issues-checkclosedstatus",
          body: data);
      if (response.statusCode == 200) {
        jsonData = json.decode(response.body);
        if (jsonData != null) {}
  ...
  keepstatus(String news, String detail,String cid) async {
    ...
    if (sharedPreferences.getString("token") != null) {
      ...
      var jsonData = null;
      var response = await http.post(
          '${ApiUrl}'+"/api/issues-checkkeepstatus",
          body: data);
      if (response.statusCode == 200) {
        jsonData = json.decode(response.body);
        if (jsonData != null) {}
